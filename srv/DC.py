...
def DC_agc_en(ip_addr='192.168.0.1', enable=''):
    """
    Записать/прочитать значение индикатора включения АРУ 
...
def DC_gain(ip_addr='192.168.0.1', gain=''):
    """
    Записать/прочитать значение коэффициента усиления сигнального канала (при отключенной АРУ) 
    @param ip - ip-адрес устройства
...
def DC_bpf(ip_addr='192.168.0.1', bpf=''):
    """
    Записать/прочитать значение полосового фильтрa 
    @param ip - ip-адрес устройства
...
def DC_freq(ip_addr='192.168.0.1', freq='', inv=''):
    """
    Записать/прочитать значение частоты настройки сигнального канала с учетом инверсии спектра
    @param ip - ip-адрес устройства
...
def DC_freqbr(ip_addr='192.168.0.1', freqbr=''):
    """
    Записать/прочитать значение частоты настройки канала наведения 
    @param ip - ip-адрес устройства
...
def DC_thrs1(ip_addr='192.168.0.1', thrs1=''):
    """
    Записать/прочитать значение нижнего порога срабатывания детектора наличия рабочего сигнала 
    @param ip - ip-адрес устройства
...
def DC_thrs2(ip_addr='192.168.0.1', thrs2=''):
    """
    Записать/прочитать значение верхнего порога срабатывания детектора наличия рабочего сигнала 
    @param ip - ip-адрес устройства
...
def DC_thrsb(ip_addr='192.168.0.1', thrsb=''):
    """
    Записать/прочитать порог срабатывания детектора наличия сигнала наведения 
    @param ip - ip-адрес устройства
